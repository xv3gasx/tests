...
local queueOnTeleport =
    ...
    (syn and syn.queueonteleport) or
    queue_on_teleport or
    queueonteleport or
    (fluxus and fluxus.queue_on_teleport) or
    (fluxus and fluxus.queueonteleport) or
    (krnl and krnl.queue_on_teleport) or
    (krnl and krnl.queueonteleport)

local AUTOEXEC_CACHE_FILE = "pls_donate_helper_autoexec.lua"
local AUTOEXEC_FILE_CANDIDATES = {
    "script.txt",
    "script.lua",
    "Pls Donate/script.txt",
    "Pls Donate/script.lua",
    "sc/Pls Donate/script.txt",
    "sc/Pls Donate/script.lua",
    "New folder/sc/Pls Donate/script.txt",
    "New folder/sc/Pls Donate/script.lua",
    AUTOEXEC_CACHE_FILE,
}
...
local function getCurrentChunkPathCandidates()
    local candidates = {}
    ...
    local function addSource(source)
        ...
        appendUnique(candidates, path:gsub("\\", "/"))
        appendUnique(candidates, path:gsub("/", "\\"))
    end
...
local function tryDecompileCurrentChunk()
    local decompilers = {
        decompile,
        syn and syn.decompile,
        syn and syn.decompile_script,
    }

    local function tryFunction(fnObj)
        if type(fnObj) ~= "function" then
            return nil
        end

        for _, dec in ipairs(decompilers) do
            if type(dec) == "function" then
                local ok, result = pcall(dec, fnObj)
                if ok and type(result) == "string" and result:find("PLS DONATE Helper", 1, true) then
                    return result
                end
            end
        end
    ...
    if debug and type(debug.info) == "function" then
        for level = 1, 8 do
            local ok, fnObj = pcall(function()
                return debug.info(level, "f")
            end)
            if ok then
                local source = tryFunction(fnObj)
                if source then
                    return source
                end
            end
    ...
    if debug and type(debug.getinfo) == "function" then
        for level = 1, 8 do
            ...
            if ok and type(info) == "table" then
                local source = tryFunction(info.func)
                if source then
                    return source
                end
            end
...
local function cacheAutoExecSource(env, sourceText, loadedPath)
    if type(sourceText) ~= "string" or sourceText == "" then
        return nil
    end

    if type(env) == "table" then
        env.__PLS_DONATE_HELPER_SOURCE = sourceText
        if type(loadedPath) == "string" and loadedPath ~= "" then
            env.__PLS_DONATE_HELPER_SOURCE_PATH = loadedPath
        end
    end

    if type(writefile) == "function" then
        pcall(writefile, AUTOEXEC_CACHE_FILE, sourceText)
    end

    return sourceText
end
local function queueTeleportChunk(chunk)
    local tried = 0
    local lastErr
    ...
    local function try(fn)
        ...
        tried = tried + 1
        local ok, result = pcall(fn, chunk)
        if ok and result ~= false then
            return true
        end

        if ok then
            lastErr = "queue_on_teleport returned false"
        else
            lastErr = result
        end
        return false
    ...
    if try(queueOnTeleport) then
        return true
    end
    if try(queue_on_teleport) then
        return true
    end
    if try(queueonteleport) then
        return true
    end
    if try(syn and syn.queue_on_teleport) then
        return true
    end
    if try(syn and syn.queueonteleport) then
        return true
    end
    if try(fluxus and fluxus.queue_on_teleport) then
        return true
    end
    if try(fluxus and fluxus.queueonteleport) then
        return true
    end
    if try(krnl and krnl.queue_on_teleport) then
        return true
    end
    if try(krnl and krnl.queueonteleport) then
        return true
    end
    ...
    return false, tostring(lastErr or "queue_on_teleport failed")
end
...
local function resolveAutoExecSource()
    local env = getgenv and getgenv() or nil

    if type(env) == "table" and type(env.__PLS_DONATE_HELPER_SOURCE) == "string" and env.__PLS_DONATE_HELPER_SOURCE ~= "" then
        return env.__PLS_DONATE_HELPER_SOURCE
    end

    local chunkCandidates = getCurrentChunkPathCandidates()
    for _, path in ipairs(chunkCandidates) do
        local raw = tryReadFile(path)
        if raw then
            return cacheAutoExecSource(env, raw, path)
        end
    end

    for _, path in ipairs(AUTOEXEC_FILE_CANDIDATES) do
        local raw = tryReadFile(path)
        if raw then
            return cacheAutoExecSource(env, raw, path)
        end
    ...
    if type(getclipboard) == "function" then
        local okClip, clip = pcall(getclipboard)
        if okClip and type(clip) == "string" and clip:find("PLS DONATE Helper", 1, true) then
            return cacheAutoExecSource(env, clip, "clipboard")
        end
    end

    local decompiled = tryDecompileCurrentChunk()
    if decompiled then
        return cacheAutoExecSource(env, decompiled, "decompile")
    end

    return nil
end
local function queueCurrentScriptOnTeleport()
    local bootstrapUrl = tostring(AUTOEXEC_HTTP_URL or "")
    bootstrapUrl = bootstrapUrl:gsub("^%s+", ""):gsub("%s+$", "")

    if bootstrapUrl ~= "" then
        if not bootstrapUrl:match("^https?://") then
            return false, "invalid AUTOEXEC_HTTP_URL"
        end

        local alternateUrl
        if bootstrapUrl:match("%.lua$") then
            alternateUrl = bootstrapUrl:gsub("%.lua$", "")
        else
            alternateUrl = bootstrapUrl .. ".lua"
        end

        if alternateUrl == bootstrapUrl then
            alternateUrl = ""
        end

        local httpChunk = string.format([[
local urls = {%s, %s}
for _, url in ipairs(urls) do
    if type(url) == "string" and url ~= "" then
        ...
        if okBody and type(body) == "string" and body ~= "" and not body:find("404: Not Found", 1, true) then
            local okRun, runErr = pcall(function()
                loadstring(body)()
            end)
            if okRun then
                warn("[PLS DONATE Helper] Teleport autoexec executed: " .. tostring(url))
            else
                warn("[PLS DONATE Helper] Teleport autoexec runtime error: " .. tostring(runErr))
            end
            break
        else
            warn("[PLS DONATE Helper] Teleport autoexec HttpGet failed: " .. tostring(url))
        end
    end
end
]], HttpService:JSONEncode(bootstrapUrl), HttpService:JSONEncode(alternateUrl))
        return queueTeleportChunk(httpChunk)
    end

    local sourceText = resolveAutoExecSource()
    if not sourceText then
        return false, "script source not found (set AUTOEXEC_HTTP_URL or save as script.txt in executor workspace)"
    end

    local loaderChunk = [[
local path = "pls_donate_helper_autoexec.lua"
if type(readfile) == "function" and type(isfile) == "function" then
    ...
    return queueTeleportChunk(loaderChunk)
end
